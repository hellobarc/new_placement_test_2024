								...
								<div class="row">
									<div class="col-xl-10 col-lg-10 col-md-10 col-sm-12 col-xs-12 mx-auto">
                                        <div>
                                            ...
                                            <form action="{{route('admin.heading-match.store.true-of-nice')}}" method="POST">
                                                @csrf
                                        ...
                                        <table class="table table-bordered table-striped">
                                            ...
                                            <tbody>
                                                @foreach ($headingMatch as $items)
                                                    <tr>
                                                        <td>{{$loop->index+1}}</td>
                                                        <td>{{$items->popularPracticeTestQuestion->question_type}}</td>
                                                        {{-- <td>{{$items->}}</td> --}}
                                                        <td>{{$items->marks}}</td>
                                                        <td>
                                                            <a href="{{route('admin.heading-match.delete.true-of-nice', ['id'=>$items->id])}}" class="btn btn-danger btn-sm">Delete</a>
                                                        </td>
                                                    </tr>
                                                @endforeach
                                            </tbody>
